...
class SaleChart(QMainWindow):
    ...
    def create_chart(self, invoice_data):
        ...
        product_names = [row[1] for row in invoice_data]  # 'name'
        total_prices = [row[0] for row in invoice_data]  # 'total_price'

        # Calcular el valor máximo para escalar las barras
        max_value = max(total_prices) if total_prices else 1
        ...
        for i in range(num_labels + 1):
            ...
            line = QGraphicsRectItem(
                x_offset, y_position, len(invoice_data) * (bar_width + spacing), 1
            )
        ...
        for i, (name, total_price) in enumerate(zip(product_names, total_prices)):
            x_position = x_offset + i * (bar_width + spacing)

            # Altura de la barra
            bar_height = (total_price / max_value) * chart_height

            # Dibujar la barra del total_price
            ...
            rect.setBrush(QBrush(QColor("#4CAF50")))  # Verde para el total_price
            ...
            value_item = QGraphicsTextItem(f"CLP{total_price:.2f}")
