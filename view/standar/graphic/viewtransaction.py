...
class TransactionChart(QMainWindow):
    def __init__(self):
        ...
        loadUi(
            f"design/standar/maingraphictransaction{self.theme}.ui", self
    ...
    def create_chart(self, transaction_data):
        ...
        transaction_dates = [
            datetime.strptime(row[0], "%Y-%m-%d") for row in transaction_data
        ]  # 'transaction_date'
        transaction_amounts = [row[1] for row in transaction_data]  # 'amount'

        # Calcular el valor máximo para escalar las barras
        max_value = max(transaction_amounts) if transaction_amounts else 1
        num_labels = 10  # Número de etiquetas en el eje Y
        increment = max_value / num_labels  # Incremento entre cada etiqueta
        ...
        for i in range(num_labels + 1):
            ...
            line = QGraphicsRectItem(
                x_offset, y_position, len(transaction_data) * (bar_width + spacing), 1
            )
            line.setBrush(QBrush(QColor("#E8E8E8")))
            line.setOpacity(1.0)
            self.scene.addItem(line)

        # Crear las barras para cada transacción
        for i, (date, amount) in enumerate(zip(transaction_dates, transaction_amounts)):
            x_position = x_offset + i * (bar_width + spacing)

            # Altura de la barra
            bar_height = (amount / max_value) * chart_height

            # Dibujar la barra de la transacción
            ...
            rect.setBrush(QBrush(QColor("#1E90FF")))  # Azul para las transacciones
